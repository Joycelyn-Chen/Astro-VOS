...
class Configuration():
    def parse(self, unknown_arg_ok=False):
        ...
        # Data parameters
        parser.add_argument('--static_root', help='Static training data root', default='../XMEM/static')
        parser.add_argument('--bl_root', help='Blender training data root', default='../BL30K')
        parser.add_argument('--yv_root', help='YouTubeVOS data root', default='../XMEM/YouTube')
        parser.add_argument('--davis_root', help='DAVIS data root', default='../XMEM/DAVIS')
        parser.add_argument('--astro_root', help='Astro training data root', default='../XMEM/astro')
        parser.add_argument('--num_workers', help='Total number of dataloader workers across all GPUs processes', type=int, default=16)
        
        #Joycelyn
        parser.add_argument('--loader_shuffle', help='Determine if you want the data to be shuffled or not in the dataloader', default = True)
        ...
        parser.add_argument('--stages', help='Training stage (0-static images, 1-Blender dataset, 2-DAVIS+YouTubeVOS, 4-astro images)', default='02')
        ...
        # Stage 4, astro images
        parser.add_argument('--s4_batch_size', default=16, type=int)
        parser.add_argument('--s4_iterations', default=150001, type=int)
        parser.add_argument('--s4_finetune', default=1000, type=int)
        parser.add_argument('--s4_steps', nargs="*", default=[], type=int)
        parser.add_argument('--s4_lr', help='Initial learning rate', default=1e-5, type=float)
        parser.add_argument('--s4_num_ref_frames', default=2, type=int)
        parser.add_argument('--s4_num_frames', default=3, type=int)
        parser.add_argument('--s4_start_warm', default=20000, type=int)
        parser.add_argument('--s4_end_warm', default=70000, type=int)
