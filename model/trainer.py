    ...
    def do_pass(self, data, it=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.config['amp']):
            ...
            for ti in range(1, self.num_frames):
                ...
                #DEBUG
                if (not it % 5000 or it == 0) and (self.save_path is not None):
                    mask_out = os.path.join(self.save_path, 'masks_output')
                    if (it == 0):
                        os.makedirs(mask_out, exist_ok=True)
                        with open(os.path.join(mask_out, "img_names.txt"), "w") as f:
                            f.write(f"it {it}: {data['info']['name'][0]}\n")
                    
                    # frames_np = frames[0, 0, :, :, :].cpu().numpy()
                    # frame_PIL = np.transpose(frames_np.astype(np.uint8), (1, 2, 0)) 
                    # img_PIL = Image.fromarray(frame_PIL, 'RGB')
                    # img_PIL.save(os.path.join(mask_out, f"{it}_img.jpg"))
                    
                    # img_name = data['info']['name'][0]
                    # print(f"Image name: {img_name}\n\n\n")

                    # img_cv = cv2.imread(img_name)
                    with open(os.path.join(mask_out, "img_names.txt"), "a+") as f:
                        f.write(f"it {it}: {data['info']['name'][0]}\n")

                    # img_np = frames[0].cpu().detach().numpy()
                    
                    # img_np = frames[0, 0, :, :, :].cpu().detach().numpy().transpose(1, 2, 0)
                    # img_np = img_np.astype(np.uint8)
                    x = self.inv_im_trans(frames[0]).permute(0,2,3,1)
                    cx = x[-1].cpu().detach().numpy()
                    yd = cx * 255
                    img_np = np.uint8(yd)
                    # im = Image.fromarray(d)
                    # im.save('x.png')
                    cv2.imwrite(os.path.join(mask_out, f"{it}_img.jpg"), img_np)

                    mask_np = masks[0, 0, :, :].cpu().detach().numpy() * 255.0
                    mask_np = mask_np.astype(np.uint8)
                    cv2.imwrite(os.path.join(mask_out, f"{it}_mask.png"), mask_np)
