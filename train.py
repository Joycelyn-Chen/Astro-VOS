...
from dataset.astro_dataset import AstroDataset

from util.logger import TensorboardLogger
from util.configuration import Configuration
from util.load_subset import load_sub_davis, load_sub_yv, load_sub_astro
...
for si, stage in enumerate(stages_to_perform):
    ...
    def construct_loader(dataset):
        train_sampler = torch.utils.data.distributed.DistributedSampler(dataset, rank=local_rank, shuffle=config['loader_shuffle'])
    ...
    def renew_astro_loader(max_skip, num_slices, finetune=False):    
        train_dataset = AstroDataset(path.join(astro_root, 'JPEGImages'), 
                            path.join(astro_root, 'Annotations'), max_jump = max_skip, is_bl = False, num_slices = num_slices, subset=None, 
                            num_frames=config['num_frames'], finetune=finetune)      # subset = load_sub_astro()

        print(f'Astro dataset size: {len(train_dataset)}')
    ...
    if stage == '0':
        ...
        astro_root = path.join(path.expanduser(config['astro_root']))
        # astro_root = path.join(path.expanduser(config['astro_root']), 'trainval', 'long-astro')
        # format: path, method (style of storing images), mutliplier
        
        # train_dataset = AstroDataset(im_root = path.join(astro_root, 'JPEGImages'), gt_root = path.join(astro_root, 'Annotations'), num_frames=config['num_frames'], max_num_obj=1, num_timestamps = 31)
        train_dataset = StaticTransformDataset(
            [
                (path.join(astro_root, 'SN_2016_0'), 1, 1),
                (path.join(astro_root, 'SN_2016_1'), 1, 1),
                (path.join(astro_root, 'SN_2016_2'), 1, 1),
                (path.join(astro_root, 'SN_21023_0'), 1, 1),
                (path.join(astro_root, 'SN_21023_1'), 1, 1),
                (path.join(astro_root, 'SN_21023_2'), 1, 1),
                (path.join(astro_root, 'SN_21023_3'), 1, 1),
                (path.join(astro_root, 'SN_21023_4'), 1, 1),
                (path.join(astro_root, 'SN_21023_5'), 1, 1),
                (path.join(astro_root, 'SN_21023_6'), 1, 1),
                (path.join(astro_root, 'SN_21023_7'), 1, 1),
                (path.join(astro_root, 'SN_21023_8'), 1, 1),
                (path.join(astro_root, 'SN_21023_9'), 1, 1),
            ], num_frames=config['num_frames'])
        
        # (path.join(astro_root, 'SN_2016_3'), 1, 1),

        train_sampler, train_loader = construct_loader(train_dataset)
        print(f'Astro dataset size: {len(train_dataset)}')

        # astro_root = path.join(path.expanduser(config['astro_root']), 'trainval')
        # train_sampler, train_loader = renew_astro_loader(0, num_slices = 50)
        # renew_loader = renew_astro_loader
    
    elif stage == '5':
        astro_root = path.join(path.expanduser(config['astro_root']), 'trainval', 'short-astro')
        
        train_dataset = AstroDataset(im_root = path.join(astro_root, 'JPEGImages'), gt_root = path.join(astro_root, 'Annotations'), num_frames=config['num_frames'], max_num_obj=1, num_timestamps = 5)
    ...
    try:
        while total_iter < config['iterations'] + config['finetune']:
            ...
            for data in train_loader:
                ...
                if stage != '0' and stage != '4'and stage != '5' and total_iter >= change_skip_iter[0]:
